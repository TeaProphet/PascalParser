...
class SemanticAnalyzer(NodeVisitor):
    ...
    def __change_type(self, type) -> str:
        for key in self.dictionary:
            if type == key:
                return self.dictionary[key]
        return type

    def __type_checker(self, type1, type2):
    ...
    def __is_binary_args_valid(self, binOP, type_var):
        for key in self.BinOpArgTypes:
            if (binOP == key and type_var in self.BinOpArgTypes[key]):
                return True
        return False

    def __binary_returning_type(self, binOP, type_var) -> str:
    ...
    def visit_BinOpNode(self, node):
        ...
        if (not isinstance(node.arg1, BinOpNode)) or not (isinstance(node.arg2, BinOpNode)):
            if type_arg1 is not None and not self.__type_checker(type_arg1, type_arg2):
                raise Exception("Incompatible types in line: ")
            type_arg1 = self.__change_type(type_arg1)
            type_arg2 = self.__change_type(type_arg2)
            if not self.__is_binary_args_valid(node.op.name, type_arg1):
                raise Exception(
                    "Operation {op} not supported for types {t1} and {t2}"
                    .format(op=node.op.name, t1=type_arg1, t2=type_arg2))
            return self.__binary_returning_type(node.op.name, type_arg1)
    ...
    def visit_AssignNode(self, node: AssignNode):
        ...
        if not self.__type_checker(type_visited, type_var):
            raise Exception(
